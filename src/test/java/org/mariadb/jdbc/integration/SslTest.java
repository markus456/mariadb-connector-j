...
public class SslTest extends Common {
  ...
  void ensureSslUnixSocket() throws SQLException {
    ...
    try (java.sql.Connection connectionSocket = createCon(baseOptions + "&sslMode=verify-ca&localSocket=" + path, sslPort)) {
      ResultSet resultSet =
              connectionSocket.createStatement().executeQuery("select 1");
