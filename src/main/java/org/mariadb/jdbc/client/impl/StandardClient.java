      ...
      switch (buf.getByte() & 0xFF) {
        ...
        case 0x00:
          ...
          if (certFingerprint != null) {
            ...
            if (!authPlugin.isMitMProof() || credential.getPassword() == null || credential.getPassword().isEmpty() || !validateFingerPrint(
                    authPlugin, okPacket.getInfo(), certFingerprint, credential, context.getSeed())) {
              throw context
                      .getExceptionFactory()
                      .create(
                              "Self signed certificates. Either set sslMode=trust, set a password or provide server certificate to client",
                              "08000");
            }
          }
